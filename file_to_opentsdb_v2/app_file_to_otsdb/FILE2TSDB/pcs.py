...
def _produce(works_to_do_list, qidx, works_done, meta):
    logger.info('데이터 처리 프로세스 (PID : %d) Start Work' %os.getpid())
    ...
    while True:
        ...
        if type(works) == type("str") and works == 'END':
            ...
            logger.info('데이터 처리 프로세스 (PID : %d) 정상 종료' %os.getpid())
            #print('Producer %d 종료\n' %os.getpid())
            break
         # 빈 데이터를 받으면 continue
...
def _consume(works_to_do, _empty1, meta, _empty2):
    pid = str(os.getpid())
    isProduceEnd = False
    logger.info('DB전송 프로세스 (PID : %s) Start Work' %pid)
    ...
    while True:
        ...
        if (_qsz % 250) == 0 :
            ...
            if isProduceEnd:
                logger.info('[Put Json to OpenTSDB] Queue Size %d ' %(_qsz))
        ...
        if works == 'END':
            #print('Consumer %s 종료\n' %pid)
            isProduceEnd = True
            logger.info('DB 전송 프로세스 (PID : %s) 정상 종료' %pid)
            break
            ...
            try:
                ...
                while (response.status_code > 204):
                    ...
                    log_msg = 'http put error' + response + __file__ + '\n' + response.text + "\n[Process ID] " + pid
                    ...
                    if tries > 10:
                        logger.error("Tried count exceed")
            ...
            # 서버로부터 아예 응답이 없는 경우
                ...
                while True:
                    if retry > 10:
                        logger.error('ERROR : NO RESPONSE FROM SERVER. EXITING THIS PROGRAM')
            ...
            # 현재는 Queue max 갯수를 full 로 체크하다가, full 되면 종료시켜 버림
                ...
                #shx.value += 1
                logger.error('###' + __file__ + 'ERROR : IOError. system exits')
...
class Workers:
    ...
    def start_work(self, meta):
        ...
        logger.info('멀티 프로세스 생성 완료 및 작업 시작..')
